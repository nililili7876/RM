...
#define SHOOTFORMULA(X,Y) (2500 - ((Y - FRAMEHEIGHT/2)*(Y - FRAMEHEIGHT/2) + (X - FRAMEWIDTH/2)*(X - FRAMEWIDTH/2)))
...
void SolveAngle::GetPoseV(const vector<Point2f>& pts, float ballet_speed, bool small)
{
    Kalman1  kalman;
    cv::Mat Rvec;
    cv::Mat_<float> Tvec;
